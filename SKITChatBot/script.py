...
if os.path.isfile("/tmp/skitchatbot.brn"):
    kernel.bootstrap(brainFile = "/tmp/skitchatbot.brn")
else:
    kernel.bootstrap(learnFiles = "aiml/std-startup.xml", commands = "load aiml b")
    #kernel.saveBrain("/tmp/skitchatbot.brn")
...
while True:
    if mode == "voice":
        ...
        response = raw_input("Talk to SKIT ChatBot : ")
    if response.lower().replace(" ","") in terminate:
        break
    jarvis_speech = kernel.respond(response)
    print "SKIT ChatBot: " + jarvis_speech
